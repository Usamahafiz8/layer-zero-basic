...
const buyAmount = 100; // Reduced amount
const stakeAmount = 50; // Reduced amount
...
async function main() {
  ...
  // Reduced LayerZero fee
  const hardcodedFee = ethers.utils.parseEther("0.005"); // Reduced from 0.01

  const txBuy = await presale.buyTokens(buyAmount, {
    value: hardcodedFee,
    gasLimit: 300000, // Reduced gas limit
    gasPrice: ethers.utils.parseUnits("25", "gwei") // Set explicit gas price
  });
